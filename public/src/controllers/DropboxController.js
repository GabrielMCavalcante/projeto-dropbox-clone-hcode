class DropboxController {
    ...
    constructor() {
        ...
        this.progressBarEl = document.querySelector('div.mc-progress-bar-fg');

        this.startUploadTime;

        this.nameFileEl = document.querySelector("span.filename");
        this.timeLeftEl = document.querySelector('span.timeleft');
    ...
    connectToFirebase() {
        const firebaseConfig = {
            apiKey: "AIzaSyBrCdl5hmFRvVTzs3G8QUyhrIS-qcX5at4",
            authDomain: "dropbox-clone-8b4f4.firebaseapp.com",
            databaseURL: "https://dropbox-clone-8b4f4.firebaseio.com",
            projectId: "dropbox-clone-8b4f4",
            storageBucket: "dropbox-clone-8b4f4.appspot.com",
            messagingSenderId: "419399122014",
            appId: "1:419399122014:web:dad086f395f2a437f688cf",
            measurementId: "G-2TR0186B2T"
        };
    ...
    {
        const fileRef = firebase.storage().ref(ref).child(fileName);
            
        return fileRef.delete();
    }

    removeFolder(ref, fileName)
    {
        return new Promise((resolve, reject)=>{

            const folderRef = this.getFirebaseRef(ref+'/'+fileName);
            folderRef.on('value', snapshot=>{
                ...
                snapshot.forEach(item=>{
                    ...
                    if(data.type === 'folder')
                    {
                        this.removeFolder(ref+'/'+fileName, data.name).then(()=>{
    ...
    initEvents() {
        ...
        this.btnNewFolder.addEventListener('click', e => {
            const folderName = prompt('Nome da nova pasta:', 'untitled');
            if (folderName) {
        ...
        this.btnRename.addEventListener('click', e => {
            ...
            const newName = prompt('Renomear o arquivo: ', file.name);
        ...
        this.inputFilesEl.addEventListener('change', e => {
            ...
            this.uploadTask(e.target.files).then(responses => {
                responses.forEach(response => {
                    ...
                    //ajax: this.getFirebaseRef().push().set(response.files['input-file']);

                    this.getFirebaseRef().push().set({
                        name: response.name,
                        type: response.contentType,
                        path: response.customMetadata.downloadURL,
                        size: response.size
                    });
                });
    ...
    modalToggle() {
        if (this.snackModalEl.style.display == 'block')
            this.snackModalEl.style.display = 'none';
        else this.snackModalEl.style.display = 'block';
    }
    ...
    uploadTask(files) {
        ...
        [...files].forEach(file => {
            ...
            promises.push(this.ajax('POST', '/upload', formData,
                e => {
                    this.uploadProgress(e.loaded, e.total, file.name);
                }, () => {
                    this.startUploadTime = Date.now();
                }, true));
            */
    ...
    /*ajax(method = 'GET', url, formData = new FormData(),
        onprogress = e => { }, onloadstart = e => { }, toggleProgress = false) {
        return new Promise((resolve, reject) => {
    ...
    uploadProgress(loaded, total, fileName) {
        ...
        this.nameFileEl.innerHTML = fileName;
    ...
    formatTime(duration) {
        ...
        else if (minutes > 0) return `${minutes} minutos e ${seconds} segundos`;

        else if (seconds > 0) return `${seconds} segundos`;

        else return '';

    }

    getFileIconView(fileType) {
        switch (fileType) {
            case 'folder':
                {
                    return `
    ...
    getFileView(file, key) {
        ...
        li.innerHTML = ` 
            ${this.getFileIconView(file.type)}
    ...
    renderNav() {
        ...
        for (let i = 0; i < this.currentFolder.length; i++) {
            ...
            else span.textContent = folderName;

            this.navEl.appendChild(span);
    ...
    initEventsLi(li) {
        ...
        li.addEventListener('click', e => {
            ...
            if (e.shiftKey) {
                ...
                let limit = [];
                const lastIndex = [...this.listFilesEl.children].indexOf(li);

                if (firstLi) {
                    const firstIndex = [...this.listFilesEl.children].indexOf(firstLi);

                    if (firstIndex < lastIndex)
                        limit = [firstIndex + 1, lastIndex];
                    else
                        limit = [lastIndex, firstIndex - 1];
                }
                else limit = [0, lastIndex];

                for (let i = limit[0]; i <= limit[1]; i++)
                    [...this.listFilesEl.children][i].classList.toggle('selected');
            ...
            if (!e.ctrlKey) {
                [...this.listFilesEl.children].forEach(item => {
